...
public class ContainerConfigTest {
	...
	public static final byte[] PRIVATE_KEY = { 45, 45, 45, 45, 45, 66, 69, 71, 73, 78, 32, 82, 83, 65, 32, 80, 82, 73, 86, 65, 84, 69, 32, 75, 69, 89, 45, 45, 45, 45, 45, 13, 10, 77, 73, 73, 69, 111, 119, 73, 66, 65, 65, 75, 67, 65, 81, 69, 65, 108,
			...
			45, 45, 45, 45, 45 };
	public static final String IP_ADDRESS = "52.209.119.244";
	public static final int PORT = 22;
	public static final String REPO_PATH = "anil2993/tomcat";
	public Logger log = LoggerFactory.getLogger(ContainerConfigTest.class);

	@Test
	public void testGetContainerIds() {
		ContainerConfig config = new ContainerConfig();
		try {
			config.getContainerIds(PRIVATE_KEY, USERNAME, IP_ADDRESS);
		} catch (ContainerException e) {
			Assert.fail("Failed to Get Container Ids");
			log.error("Unable to connect to VM to get container id", e);
		}
	}
	...
	public void testGetContainerStats() {
		...
		try {
			containerIds = config.getContainerIds(PRIVATE_KEY, USERNAME, IP_ADDRESS);

			for (String containerId : containerIds) {
				config.getContainerStats(PRIVATE_KEY, USERNAME, IP_ADDRESS, containerId);
			}
		} catch (ContainerException e) {
			Assert.fail("Unable to get Container Stats");
			log.error("Unable to Get container Stats", e);
		}
	}
	...
	public void testStartContainers() {
		...
		boolean isDockerServiceRunning = false;
		try {
			isDockerServiceRunning = vmConfig.checkForDockerService(PRIVATE_KEY, USERNAME, IP_ADDRESS);

			if (isDockerServiceRunning) {
				config.startContainers(PRIVATE_KEY, USERNAME, IP_ADDRESS, REPO_PATH);
			}
		} catch (DockerException | ContainerException e) {
			Assert.fail("Failed to Start Container");
	...
	public void testStopContainers() {
		...
		try {
			containerIds = config.getContainerIds(PRIVATE_KEY, USERNAME, IP_ADDRESS);
			config.stopContainers(PRIVATE_KEY, USERNAME, IP_ADDRESS, containerIds);
		} catch (ContainerException e) {
			Assert.fail("Failed to Stop Containers");
			log.error("Failed to Stiop Containers", e);
		}
	}
}
